"""
This script provides a GUI to monitor a directory for new image stack folders,
detect streaks in the images using OpenCV, and output both MP4 and GIF clips
of detected streaks for further analysis.
"""
...
class DetectStreaksGUI:
    """GUI for monitoring image stack folders and detecting streaks."""

    def __init__(self, master):
        """Initialize the GUI layout and internal state."""
        self.root = master
        ...
        Label(master, text="Top-Level Recordings Folder:").pack(pady=5)
        Button(master, text="Select Folder", command=self.select_folder).pack()

        self.status_label = Label(master, text="No folder selected.")
        self.status_label.pack(pady=5)

        self.toggle_button = Button(
            self.root, text="Start Watching", command=self.toggle_monitoring
        )
        self.toggle_button.pack(pady=10)

        self.log = Text(self.root, height=15, width=80)
        self.log.pack(side="left", padx=(10, 0), pady=5)

        scrollbar = Scrollbar(master, command=self.log.yview)
        scrollbar.pack(side="right", fill="y")
        self.log.config(yscrollcommand=scrollbar.set)

    def wait_until_stable(self, folder_path):
        """Wait until the contents of a folder stop changing in size."""
        self.log_message(
            f"[WAIT] Checking stability for: {os.path.basename(folder_path)}"
        )
        last_size = -1
        stable_count = 0
    ...
    def select_folder(self):
        """Prompt user to select a directory to monitor."""
        folder = filedialog.askdirectory()
        if folder:
            self.base_dir.set(folder)
            self.status_label.config(
                text=f"Monitoring latest folder in: {folder}"
            )

    def toggle_monitoring(self):
        """Toggle the monitoring state of the GUI."""
    ...
    def start_monitoring(self):
        """Start a background thread to monitor for new folders."""
    ...
    def get_latest_dated_subfolder(self):
        """Return the most recent date-named subfolder."""
        try:
            self.log_message(
                f"[DEBUG] Looking for date folders in: {self.base_dir.get()}"
            )
            self.log_message(f"[DEBUG] Found: {os.listdir(self.base_dir.get())}")
            dated_folders = [
                d for d in os.listdir(self.base_dir.get())
                if os.path.isdir(os.path.join(self.base_dir.get(), d))
    ...
    def is_valid_date_folder(folder_name):
        """Check if folder name matches date format MM-DD-YYYY."""
        try:
    ...
    def monitor_loop(self):
        """Continuously check for new image stack folders."""
        seen = set()
    ...
    def detect_streaks(self, stack_folder, frames_folder):
        """Run streak detection on stacked image files."""
        ...
        detection_folder = os.path.join(
            os.path.dirname(stack_folder), "Detections", "Clips"
        )
        ...
        for i in range(1, len(stack_files)):
            ...
            self.log_message(f"[DEBUG] Lines found: {count}")

            if lines is not None:
                start = (i - 1) * 20 + 1
                end = i * 20
                mp4_out = os.path.join(
                    detection_folder, f"clip_{start:05d}_{end:05d}.mp4"
                )
                gif_out = os.path.join(
                    detection_folder, f"clip_{start:05d}_{end:05d}.gif"
                )
                ...
                subprocess.run([
                    ...
                    "-start_number", str(start), "-i", pattern,
                ...
                subprocess.run([
                    "ffmpeg", "-y", "-framerate", "20",
                    "-start_number", str(start), "-i", pattern,
                    "-vframes", str(end - start + 1), gif_out
                ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)

                self.log_message(f"[SAVED] {os.path.basename(gif_out)}")

    def log_message(self, message):
        """Insert a timestamped message into the log window."""
