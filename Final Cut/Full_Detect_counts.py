...
MIN_CONTOUR_AREA    = 20      # px² — ignore tiny noise blobs
...
def run_detection_mkv(mkv_path, mask_path, master,
    ...
    clips_dir = os.path.join(os.path.dirname(mkv_path), f"{base_name}_events")
    ...
    # Ring buffer holds (frame_index, gray_frame, overlay, count) tuples
    ...
    while True:
        ...
        entry = (frame_idx, gray, overlay, count)
        ...
        if count > 0:
            ...
            if post_countdown == 0:
                # Flush pre-buffer into pending_save
            ...
            if post_countdown == 0:
                # Write this event clip to disk
                for (fidx, gframe, foverlay, fcount) in pending_save:
                    ...
                    saved_counts.append(fcount)
                    saved_overlays.append(opath)
                pending_save.clear()
    ...
    try:
        ...
        if len(sys.argv) < 3:
            messagebox.showerror(
                ...
                "Pass empty string for mask_path to run without a mask."
            )
            sys.exit(1)

        input_path = sys.argv[1]
        mask_path  = sys.argv[2] if sys.argv[2] else None
