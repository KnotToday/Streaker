...
class Stack_Builder_Tester_GUI:  # main GUI class for generating stack sweeps and detection clips
    def __init__(self, root):
        ...
        Label(root, text="Select Frames Folder:").pack(pady=5)  # prompt to choose input folder
        Button(root, text="Choose Folder", command=self.select_folder).pack()

        self.status_label = Label(root, text="No folder selected.")  # dynamic label for folder path
        self.status_label.pack(pady=5)

        self.start_frame_label = Label(root, text="Start from Frame Filename:")  # prompt to select start frame
        self.start_frame_label.pack()
        self.filename_combobox = ttk.Combobox(root, textvariable=self.start_filename, state="readonly", height=20) # dropdown for available .png filenames
        self.filename_combobox.pack()

        Label(root, text="Frames per Stack:").pack()  # slider label
        Scale(root, from_=2, to=40, orient=HORIZONTAL, variable=self.stack_group_size).pack() # user sets how many frames are used per stack

        Label(root, text="Select Stacking Methods:").pack()  # label for checkboxes
        self.method_vars = {"max": IntVar(value=1),"mean": IntVar(value=0),"median": IntVar(value=0),"min": IntVar(value=0),}
        for method, var in self.method_vars.items():
            Checkbutton(root, text=method, variable=var).pack(anchor="w")  # user can select stacking method(s)

        Button(root, text="Generate Sweep Samples", command=self.start_stack_thread).pack(pady=10)
        Button(root,text="Create Detection Clip (1 min)",command=self.generate_detection_clip,).pack(pady=2)
    ...
    def generate_detection_clip(
        ...
        timestamp = datetime.now().strftime("%m-%d-%Y_%H-%M-%S")  # unique timestamp for output folder
        output_dir = os.path.join(sweep_folder, f"Detection_Clip_{stem}_start_{start_number}_{timestamp}")
        frames_output = os.path.join(output_dir, "Frames")  # subfolder to hold copied frames
        stack_output = os.path.join(output_dir, "Stacked")  # subfolder to hold stacked images
        ...
        stack_method = "max"
        stack_group_size = (self.stack_group_size.get())  # get frames-per-stack from slider
        stacked_frames = []
        for i in range(
            0, len(frame_paths), stack_group_size):  # process each stack group
            ...
            if stack_method == "max":
                ...
                continue
            cv2.imwrite(os.path.join(stack_output, f"{i//stack_group_size+1:04d}.png"),out,)

        metadata = {"start_frame": start_number,"stack_method": stack_method,"stack_group_size": stack_group_size,"frame_count": len(frame_paths),}
        with open(os.path.join(output_dir, "clip_info.txt"), "w") as f:json.dump(metadata, f, indent=2) # save metadata about this run
        self.log_message("[CLIP] Creating video clip...")

        cmd = ["ffmpeg","-y","-framerate","20","-start_number",str(start_number),"-i",os.path.join(frames_output, "%05d.png"),"-vframes",str(1200),
            "-c:v",
            "libx264",
            "-pix_fmt",
            "yuv420p",
            os.path.join(output_dir, f"detection_clip_start_{start_number}.mp4"),] # ffmpeg command to encode copied frames into a .mp4
        try:
            subprocess.run(cmd, check=True)  # run ffmpeg to make clip
            self.log_message("[CLIP] Detection clip saved. Launching detection test...")
            try:
                subprocess.Popen(
                    ["python", "detect_sandbox_gui.py", output_dir]
                )  # launch detect sandbox GUI
    ...
    def log_message(self, message):  # helper to append messages to GUI log with timestamp
